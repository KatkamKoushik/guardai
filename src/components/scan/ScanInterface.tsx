...
interface ScanResult {
  ...
  score: number;
  details: {
    virusTotal: { status: string; detections: number; total: number };
    googleSafeBrowsing: { status: string };
    ...
    geoIp?: { query?: string; city?: string; country?: string; isp?: string } | null;
      ...
      <div className="w-full max-w-5xl">
          ...
          <div className="flex gap-3">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-6 py-4 bg-black/50 border border-white/10 rounded-xl text-white placeholder-white/30 focus:outline-none focus:border-[#00F0FF]/50 focus:ring-1 focus:ring-[#00F0FF]/20 transition-all duration-300 font-mono"
                disabled={isScanning}
              />
            </div>
            <MagneticButton
        ...
        <GlassCard hover3D={false} className="border-[#00F0FF]/20 bg-black/80">
          <div className="flex justify-between items-center mb-4 pb-4 border-b border-white/10">
            <div className="flex items-center gap-4">
              ...
              <span className="text-xs text-white/50 font-mono">guard-ai_terminal_v2.0 // STATUS: {isScanning ? "ACTIVE_SCAN" : "IDLE"}</span>
            </div>
          ...
          <div className="h-[250px] overflow-y-auto font-mono text-xs custom-scrollbar">
            {logs.length === 0 ? (
              ...
              <div className="space-y-2">
                {logs.map((l) => (
                  ...
                  >
                    ...
                    <span className="text-[#00F0FF]/60 shrink-0 w-[130px]">{l.step}</span>
                    <span className={`shrink-0 w-[85px] ${
        ...
        <AnimatePresence>
          {result && (
            ...
            >
              <div ref={reportRef} className="bg-[#050505] p-6 rounded-2xl border border-white/10 relative">
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4">
                    <div>
                      <h3 className="text-2xl font-bold mb-1" style={{ fontFamily: "var(--font-heading)" }}>DIAGNOSTIC REPORT</h3>
                      <div className="text-sm font-mono text-white/50">{result.url}</div>
                    </div>
                    <button 
                      type="button"
                      onClick={exportPDF}
                      className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm font-mono transition-colors flex items-center gap-2"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <motion.div 
                      ...
                      className="p-6 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-center items-center text-center col-span-1"
                    >
                      <div className="relative w-24 h-24 mb-4">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <span className="text-2xl font-bold" style={{ color: threatColor }}>
                            {result.score}
                          </span>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="text-xs text-white/40 font-mono">OVERALL RISK SCORE</div>
                      </div>
                    </motion.div>

                    {/* Threat Intel (VT + GSB) */}
                    <div className="p-6 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-center">
                      <div className="text-xs text-white/40 mb-4 font-mono">THREAT INTELLIGENCE</div>
                      
                      <div className="flex flex-col gap-4">
                        {/* VirusTotal */}
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold">VirusTotal</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-mono">{result.details.virusTotal.detections}/{result.details.virusTotal.total}</span>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold">Google Safe Browsing</span>
                          </div>
                          <div className="flex items-center gap-2">
                    ...
                    {/* ML Heuristics */}
                    <div className="p-6 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-center">
                      <div className="text-xs text-white/40 mb-3 font-mono">ML LEXICAL PROBABILITY</div>
                      <div className="mb-4">
                      ...
                      <div className="space-y-1 max-h-24 overflow-y-auto custom-scrollbar pr-2">
                        {result.details.phishing.indicators.length > 0 ? (
                          ...
                          <div className="text-xs text-white/40">No suspicious indicators detected.</div>
                        )}
                      </div>
                    </div>

                    {/* IP & Geo Intelligence */}
                    {result.details.geoIp && (
                      <div className="p-6 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-center">
                        <div className="text-xs text-white/40 mb-3 font-mono">IP & GEO INTELLIGENCE</div>
                        <div className="flex flex-col gap-2">
                          <div className="text-lg font-mono text-white/80">{result.details.geoIp.query || "Unknown"}</div>
                          <div className="text-sm flex justify-between border-b border-white/5 pb-1">
                            <span className="text-white/40">Location</span> 
                            <span className="font-medium text-white/80">{result.details.geoIp.city || "Unknown"}, {result.details.geoIp.country || "Unknown"}</span>
                          </div>
                          <div className="text-sm flex justify-between pt-1">
                            <span className="text-white/40">ISP</span> 
                            <span className="truncate max-w-[150px] text-right font-medium text-white/80" title={result.details.geoIp.isp}>{result.details.geoIp.isp || "Unknown"}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {result.details.securityHeaders && (
                      <div className="p-6 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-center">
                        <div className="text-xs text-white/40 mb-3 font-mono">HTTP SECURITY HEADERS</div>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-white/60">Strict-Transport-Security</span>
                            {result.details.securityHeaders.hsts ? (
                              <span className="text-[#00FF66]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></span>
                            ) : (
                              <span className="text-[#FF003C]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></span>
                            )}
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-white/60">X-Frame-Options</span>
                            {result.details.securityHeaders.xFrameOptions ? (
                              <span className="text-[#00FF66]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></span>
                            ) : (
                              <span className="text-[#FF003C]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></span>
                            )}
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-white/60">Content-Security-Policy</span>
                            {result.details.securityHeaders.csp ? (
                              <span className="text-[#00FF66]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></span>
                            ) : (
                              <span className="text-[#FF003C]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></span>
                            )}
                    ...
                    {/* SSL */}
                    <div className="p-6 rounded-xl bg-white/5 border border-white/5 flex flex-col justify-center">
                      <div className="text-xs text-white/40 mb-3 font-mono">SSL / TLS CERTIFICATE</div>
                      <div className="flex items-center gap-3 mb-4">
                        <div className={`w-3 h-3 rounded-full ${result.details.ssl.valid ? 'bg-[#00FF66]' : 'bg-[#FF003C]'}`} />
                        <div className="text-lg font-medium">{result.details.ssl.issuer}</div>
                      </div>
                      <div className="text-xs text-white/50 font-mono">EXPIRES: {result.details.ssl.expiry}</div>
                    </div>
